...
static void ICACHE_FLASH_ATTR disconnect_callback(void * arg)
{
	...
	if(conn->reverse != NULL) {
		...
		if (req->buffer != NULL) {
			...
			char * vers_pos = (char *)os_strstr(req->buffer, "\r\n") + 2;
			int tmp_len = body - vers_pos;

			os_memcpy(headers, vers_pos, tmp_len);
			headers[tmp_len] = '\0';

			if (req->user_callback != NULL) { // Callback is optional.
				req->user_callback(body, http_status, req->buffer, headers);
			}
			os_free(req->buffer);
		}
